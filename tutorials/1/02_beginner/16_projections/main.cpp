...
GLuint VAO, VBO, IBO, gShader, gUniformProjection;
...
int main() {
  ...
  while (!glfwWindowShouldClose(gWindow)) {
    ...
    model = glm::rotate(model, gCurAngle * kToRadians, glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::scale(model, glm::vec3(0.4f, 0.4f, 1.0f));

    glUniformMatrix4fv(gUniformModel, 1, GL_FALSE, glm::value_ptr(model));
    glUniformMatrix4fv(gUniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
...
void CompileShaders() {
  ...
  gUniformProjection = glGetUniformLocation(gShader, "projection");
}